...
VITE_API_BASE_URL=
VITE_PASSKEY_REQUIRED=$PASSKEY_REQ
ENVEOF
echo "  Wrote frontend/.env.local"

# Export the real API URL so vite.config.ts can set up a preview proxy.
# With VITE_API_BASE_URL empty the browser makes same-origin /api/* calls,
# and Vite preview proxies them to the deployed API Gateway — avoiding CORS
# issues on beta/prod where FRONTEND_ORIGIN is locked to the CloudFront domain.
...
E2E_PASSKEY_REQUIRED="$PASSKEY_REQ" \
