#!/usr/bin/env bash
# deploy-ui.sh — Build and deploy the PassVault frontend (or start a local dev server).
#
# Usage:
#   ./scripts/deploy-ui.sh --env <dev|beta|prod> [--profile <aws-profile>] [--region <region>] [--stack <stack-name>]
#
# Environments:
...
{
  echo "# Auto-generated by scripts/deploy-ui.sh (--env $ENV) — DO NOT COMMIT (covered by .gitignore)"
  ...
  echo "VITE_VIEW_TIMEOUT_SECONDS=$VITE_VIEW_TIMEOUT_SECONDS"
  echo "VITE_EDIT_TIMEOUT_SECONDS=$VITE_EDIT_TIMEOUT_SECONDS"
  echo "VITE_ADMIN_TIMEOUT_SECONDS=$VITE_ADMIN_TIMEOUT_SECONDS"
} > "$ENV_FILE"
