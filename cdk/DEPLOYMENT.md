...
Note: `scripts/deploy-ui.sh` runs `init-admin.ts` automatically if the admin account is absent, so manual initialization is not required when using that script.

### 6.2 Build and Deploy Frontend

**Option A: Using `deploy-ui.sh` (recommended)**

```bash
./scripts/deploy-ui.sh --env prod --profile my-profile
```

This handles admin initialization, env file generation, build, S3 sync, CloudFront invalidation, and smoke tests in one step. See `./scripts/deploy-ui.sh --help` for all options.
